...
namespace DllRewrite
{
    internal class MethodPatcher
    {
        ...
        private readonly AssemblyDefinition StardewModdingAPI;
        ...
        public MethodPatcher()
        {
            ...
            this.StardewValley =
                this.resolver.Resolve(new AssemblyNameReference("StardewValley", new Version("1.3.0.0")));
            this.StardewModdingAPI =
                this.resolver.Resolve(new AssemblyNameReference("StardewModdingAPI", new Version("0.0.0.0")));
        }
        ...
        public MethodReference GetMethodReference(string name, string typename, AssemblyDefinition assemblyDefinition)
        {
            ...
            MethodReference method = assemblyDefinition.MainModule.Types
                .FirstOrDefault(item => item.FullName == typename).Methods.FirstOrDefault(item => item.Name == name);
            if (assemblyDefinition.FullName != this.StardewValley.FullName)
        ...
        public void ApplyGamePatch()
        {
            ...
            var jointPoint = constructor.Body.Instructions[0];
            instructions.Add(processor.Create(OpCodes.Newobj,
                this.GetMethodReference(".ctor", "SMDroid.ModEntry", this.StardewModdingAPI)));
            instructions.Add(processor.Create(OpCodes.Stsfld,
                this.GetFieldReference("hooks", "StardewValley.Game1", this.StardewValley)));
            this.InsertInstructions(processor, jointPoint, instructions);
            var updateMethod = typeGame1.Methods.FirstOrDefault(m => m.Name == "Update");
            jointPoint = updateMethod.Body.Instructions.First(ins => ins.OpCode == OpCodes.Call);
            var jointPointA = updateMethod.Body.Instructions.First(ins => ins.OpCode == OpCodes.Call).Previous.Previous;
            processor = updateMethod.Body.GetILProcessor();
            processor.InsertBefore(jointPointA, processor.Create(OpCodes.Ldsfld, this.GetFieldReference("IsHalt", "SMDroid.ModEntry", this.StardewModdingAPI)));
            processor.InsertBefore(jointPointA, processor.Create(OpCodes.Brtrue_S, jointPoint.Next));
            ...
            string qualifyNameSuffix = "", FieldReference hooksField = null, MethodReference prefixHook = null, MethodReference postfixHook = null)
        {
            ...
            if(hooksField == null)
                hooksField = this.GetFieldReference("Instance", "SMDroid.ModEntry", this.StardewModdingAPI);
            ...
            this.InsertModHook("OnGame1_CreateContentManager_Prefix", new[]
                {
                    ...
                    this.GetTypeReference("System.String"),
                    new ByReferenceType(this.GetTypeReference("StardewValley.LocalizedContentManager",
                        this.StardewValley))
                },
        ...
        public AssemblyDefinition InsertMonoHooks()
        {
            ...
            FieldReference hooksField = this.MonoGame_Framework.MainModule.ImportReference(this.GetFieldReference("Instance", "SMDroid.ModEntry",
                this.StardewModdingAPI));
            var typeModHooksObject = this.StardewValley.MainModule.GetType("StardewValley.ModHooks");
